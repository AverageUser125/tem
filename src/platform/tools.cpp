...
void assertFuncInternal(const char* expression, const char* file_name, unsigned const line_number,
	...
	int const action =
		MessageBox(0, c, "Platform Layer", MB_TASKMODAL | MB_ICONHAND | MB_ABORTRETRYIGNORE | MB_SETFOREGROUND);
...
void assertFuncProduction(const char* expression, const char* file_name, unsigned const line_number,
	...
	std::cout << "Assertion failed: " << expression << " file: " << file_name << " line: " << line_number
			  << "Comment: " << comment;
	assert(0);
	raise(SIGABRT);
}
